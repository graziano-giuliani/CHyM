...
module mod_cellaut
  ...
  subroutine cellularrule(dindex,v)
    ...
    if ( dindex<0 ) then
      ...
      if ( iflg(1)>=10 ) write (6,'(7x,a)') &
                  'MVLib cellularrule: all rules resetted to 0 value.'
      if ( dindex==-1 ) then
        if ( iflg(1)>=10 ) write (6,'(7x,a)') &
                      'MVLib cellularrule: Defining "Life Game" rules.'
        do i = 0 , 255
        ...
        if ( iflg(1)>=10 ) write (6,'(7x,a)') &
             'MVLib cellularrule: Defining rules for signal filtering.'
        irule(66+1) = 12
        irule(68+1) = 13
        irule(65+1) = 11
        do i = 0 , 255
    ...
    if ( dindex>255 .or. v>2 .or. v<-1 ) then
      write (6,'(7x,a,2i4)') 'MVLib cellularrule: bad values ' , dindex , v
    else
      irule(dindex+1) = v
    end if
  ...
  subroutine cellularinit(mat,m,n,xtype,xc,yc)
    ...
    if ( xtype==1 ) then
      ...
      write (6,'(7x,a,i10)') 'MVLib cellulardef: Uknown type: ' , xtype
      call exit(0)
    end if
  ...
  subroutine cavectorfill(v,w,z,ndat)
    ...
    if ( nrep==0 ) then
      ...
      write (6,'(10x,a,i5,a,i5,a)') 'CAVectorFill routine: no good data '//  &
                                    'found, cannot repair the vector.'
    else
      if ( iflg(1)>=10 ) then
        write (str,'(a,i8,a,i8,a)') 'CAVectorFill routine: ' , nrep ,        &
                                    ' components of ' , ndat ,               &
                                    ' contains -9999'
        call no2space(str)
        write (6,'(10x,a)') trim(str)
        write (str,'(a,i8)') 'Maximum distance between valid components:' ,  &
                      maxd
        call no2space(str)
