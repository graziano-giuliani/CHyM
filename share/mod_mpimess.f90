...
!  For NorthEast and SouthEast directions
        ...
        if (.not.ma%has_bdytop) then
          ide2nega = ide2nega + 1
          ide2ne = ide2ne + 1
!          print*,"id NorthEasthas_bdytop,ide2ne,ide2nega",myid,ide2ne,ide2nega
        end if
        if (.not.ma%has_bdyright) then
          jde2nega = jde2nega + 1
          jde2ne = jde2ne + 1
!          print*,"id NorthEasthas_bdyright,jde2ne,jde2nega",myid,jde2ne,jde2nega
        end if
...
!        print*,"myid,ide1w,ide2w,jde1w,jde2w,nnwga",myid,ide1w,ide2w,jde1w,jde2w,nnwga
...
!  For North-South directions
        ...
        ide2lga = ide2ga
!        if (.not.ma%has_bdyright) then
!          print*,"id nordsud",myid
          jde2lga = jde2lga + 1
          jde2l = jde2l + 1
!        end if
!        print*,"myid,ide1,ide2,jde1,jde2",myid,ide1,ide2,jde1,jde2
!        if (ma%has_bdyleft) then
!          print*,"id nordsud",myid
!          jde2lga = jde2lga + 1
!          jde2l = jde2l + 1
...
!      print*,"1)global_dot_istart",global_dot_istart,"global_dot_jstart",global_dot_jstart,"myid :",myid
...
!      print*,"jxp",jxp,"iyp",iyp
!      print*,"2)global_dot_istart",global_dot_istart,"global_dot_jstart",global_dot_jstart,"myid :",myid
      ...
      subroutine mypack_real_grid(matrix,vector,i1,i2,j1,j2,disp)
        ...
        iv = 1+disp
!        print*,'myid',myid,'i1,i2,j1,j2',i1,i2,j1,j2,"disp: ",disp
        do i = i1,i2
          do j = j1,j2
...
!        print*,'myid',myid,'i1,i2,j1,j2',i1,i2,j1,j2,"disp: ",disp
      ...
      subroutine myunpack_real_grid(vector,matrix,i1,i2,j1,j2)
        ...
        iv = 1
!        print *,'myid',myid,lbound(matrix,1),ubound(matrix,1)
!        print *,'myid',myid,lbound(matrix,2),ubound(matrix,2)
!        print*,'myid',myid,'i1,i2,j1,j2',i1,i2,j1,j2
        do i = i1,i2
          do j = j1,j2
!            print*,"vector ",iv," :",vector(iv)
...
!            print*,"matrix ",j,i," :",matrix(j,i)
      ...
      subroutine exchange_bdy_real8(ml,nex,j1,j2,i1,i2,jj1,jj2,ii1,ii2)
        ...
        isize = i2-i1+1
        jsize = j2-j1+1
!        print*,myid,"NOGA",j1,j2,i1,i2
!        print*,myid,"GA",jde1ga,jde2ga,ide1ga,ide2ga
!        print*,"lbound(ml,1),ubound(ml,1)",lbound(ml,1),ubound(ml,1)
!        print*,"lbound(ml,2),ubound(ml,2)",lbound(ml,2),ubound(ml,2)
...
!          deallocate(r8vector1)
!          deallocate(r8vector2)
        end if 
...
!          deallocate(i4vector1)
!          deallocate(i4vector2)
        end if 
        if ( ma%left /= mpi_proc_null ) then
          ssize = isize
          if (allocated(i4vector1)) deallocate(i4vector1)
